...
(def splice-unquote-form
  (pa/let-bind [_ (token "~@")]
    (pa/map
      (fn [f]
        (list (new Symbol "splice-unquote") f))
      (form))))
...
(def with-meta-form
  (pa/let-bind [_ (token \^)
                m (form)
                f (form)]
    (pa/return (list (new Symbol "with-meta") f m))))
