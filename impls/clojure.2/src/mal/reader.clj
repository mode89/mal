...
(def quasiquote-form
  (pa/let-bind [_ (token \`)]
    (pa/map
      (fn [f]
        (list (types/symbol "quasiquote") f))
      (form))))
...
(def splice-unquote-form
  (pa/let-bind [_ (token "~@")]
    (pa/map
      (fn [f]
        (list (types/symbol "splice-unquote") f))
      (form))))
...
(def with-meta-form
  (pa/let-bind [_ (token \^)
                m (form)
                f (form)]
    (pa/return (list (types/symbol "with-meta") f m))))
