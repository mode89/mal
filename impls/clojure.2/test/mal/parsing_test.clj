...
(deftest return
  (let [p (partial pa/parse (pa/return 42))]
    (is (= (p '()) (success 42 '())))
    (is (= (p '(1)) (success 42 '(1))))))

(deftest fail
  (let [p (partial pa/parse (pa/fail "nope"))]
    (is (= (p '()) (failure "nope" '())))
    (is (= (p '(1)) (failure "nope" '(1))))))

(deftest label
  (let [p (partial pa/parse (pa/label "foo" (element 42)))]
    (is (= (p '(42)) (success 42 '())))
    (is (= (p '(43)) (failure "foo" '(43))))))

(deftest bind
  (let [p (partial pa/parse
            (pa/bind any-element
              (fn [x]
                (pa/bind any-element
                  (fn [y]
...
(deftest map-parser
  (let [p (partial pa/parse (pa/map inc any-element))]
    (is (= (p '()) (failure "any-element" '())))
    (is (= (p '(1)) (success 2 '())))
    (is (= (p '(2)) (success 3 '())))
    (is (= (p '(42)) (success 43 '())))
    (is (= (p '(4 42)) (success 5 '(42)))))
  (let [p (partial pa/parse (pa/map any-element :to 42))]
    (is (= (p '()) (failure "any-element" '())))
    (is (= (p '(1)) (success 42 '()))))
...
(deftest satisfy
  (let [p (partial pa/parse (pa/satisfy odd? any-element))]
    (is (= (p '(1)) (success 1 '())))
    (is (= (p '(2)) (failure "Does not satisfy predicate" '())))))

(deftest try
  (let [p (partial pa/parse (pa/try (pair 1 2)))]
    (is (= (p '(1 2)) (success [1 2] '())))
    (is (= (p '(1 3)) (failure "element: 2" '(1 3))))))

(deftest alt
  (let [p (partial pa/parse (pa/alt (element 1) (element 2)))]
    (is (= (p '(1)) (success 1 '())))
    (is (= (p '(2)) (success 2 '())))
    (is (= (p '(3)) (failure "element: 1 or element: 2" '(3)))))
  (let [p (partial pa/parse (pa/alt (pair 1 2) (pair 3 4)))]
...
(deftest many
  (let [p (partial pa/parse (pa/many (element 1)))]
    (is (= (p '()) (success [] '())))
    (is (= (p '(2)) (success [] '(2))))
    (is (= (p '(1)) (success [1] '())))
    (is (= (p '(1 1)) (success [1 1] '())))
    (is (= (p '(1 1 2)) (success [1 1] '(2)))))
  (let [p (partial pa/parse (pa/many (pair 1 2)))]
  ...
  (let [p (partial pa/parse (pa/many (element 1) :till (element 42)))]
